 ...
 */
async function connectWs(url, headers, openTimeout = WS_OPEN_TIMEOUT) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.on("error", (err) => {
            if (settled)
                return;
            settled = true;
            reject(new LangSmithSandboxConnectTimeoutError(`Failed to connect to sandbox WebSocket: ${err.message}`));
        });
    });
}
/**
...
export async function runWsStream(dataplaneUrl, apiKey, command, options = {}) {
    const { timeout = 60, env, cwd, shell = "/bin/bash", onStdout, onStderr, commandId, idleTimeout = 300, killOnDisconnect = false, ttlSeconds = 600, pty, headers: extraHeaders, openTimeout = WS_OPEN_TIMEOUT, } = options;
