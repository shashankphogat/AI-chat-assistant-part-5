...
exports.WSStreamControl = exports.WS_CONNECT_BUDGET = exports.WS_OPEN_TIMEOUT = void 0;
exports.connectDeadline = connectDeadline;
exports.remainingBudget = remainingBudget;
exports.openTimeoutFor = openTimeoutFor;
exports.isWsAvailable = isWsAvailable;
...
function envTimeout(name, defaultSeconds) {
    const raw = (0, env_js_1.getLangSmithEnvironmentVariable)(name);
    if (raw === undefined || raw.trim() === "")
        return defaultSeconds;
    const value = Number(raw);
    if (!Number.isFinite(value)) {
        console.warn(`Ignoring invalid LANGSMITH_${name}=${raw}, using ${defaultSeconds}`);
        return defaultSeconds;
    }
    return value > 0 ? value : undefined;
}
// A slow cold start is recovered by the retry loop in run(), not by waiting here.
exports.WS_OPEN_TIMEOUT = envTimeout("SANDBOX_WS_TIMEOUT_OPEN", 30);
...
function connectDeadline() {
    return exports.WS_CONNECT_BUDGET === undefined
        ? undefined
        : nowSeconds() + exports.WS_CONNECT_BUDGET;
}
...
function openTimeoutFor(deadline) {
    if (deadline === undefined)
        return exports.WS_OPEN_TIMEOUT;
    const remaining = Math.max(deadline - nowSeconds(), 0);
    return exports.WS_OPEN_TIMEOUT === undefined
        ? remaining
        : Math.min(exports.WS_OPEN_TIMEOUT, remaining);
}
...
const wsWebSocketPromise = import("ws").then((ws) => ws.default || ws.WebSocket || ws, () => null);
/** Whether the optional `ws` package could be loaded (resolved once). */
async function isWsAvailable() {
 ...
 */
async function connectWs(url, headers, openTimeout = exports.WS_OPEN_TIMEOUT) {
    const { WebSocket: WS } = await ensureWs();
    return new Promise((resolve, reject) => {
        const ws = new WS(url, {
            headers,
            handshakeTimeout: openTimeout === undefined ? undefined : openTimeout * 1000,
        });
        let settled = false;
        ws.on("open", () => {
            if (settled)
        ...
        ws.on("error", (err) => {
            if (settled)
                return;
            settled = true;
            reject(new errors_js_1.LangSmithSandboxConnectTimeoutError(`Failed to connect to sandbox WebSocket: ${err.message}`));
        });
    });
}
/**
...
async function runWsStream(dataplaneUrl, apiKey, command, options = {}) {
    const { timeout = 60, env, cwd, shell = "/bin/bash", onStdout, onStderr, commandId, idleTimeout = 300, killOnDisconnect = false, ttlSeconds = 600, pty, headers: extraHeaders, openTimeout = exports.WS_OPEN_TIMEOUT, } = options;
